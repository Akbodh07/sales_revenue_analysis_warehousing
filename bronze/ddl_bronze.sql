...
Script purpose:
	This scipt creates a new database named 'sales_revenue_analysis' after checking if it already exists.
...
-- USE master
IF EXISTS (SELECT 1 FROM sys.databases WHERE name = 'sales_revenue_analytics')
BEGIN
    ALTER DATABASE sales_revenue_analytics SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE sales_revenue_analytics;
END;
GO
-- create a 'sales_revenue_analytics' database
CREATE DATABASE sales_revenue_analytics;
